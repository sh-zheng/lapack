*> \brief <b> DSKEWSTEVX computes the eigenvalues and, optionally, the left and/or right eigenvectors for OTHER matrices</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> Download DSKEWSTEVX + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dskewstevx.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dskewstevx.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dskewstevx.f">
*> [TXT]</a>
*
*  Definition:
*  ===========
*
*       SUBROUTINE DSKEWSTEVX( JOBZ, RANGE, N, E, VL, VU, IL, IU, ABSTOL,
...
*> \verbatim
*>
*> DSKEWSTEVX computes selected eigenvalues and, optionally, eigenvectors
...
*> \verbatim
*>
*>  Derived from subroutine dstevx.
*>
*> \endverbatim
*
*> \par Contributors:
...
*>
*  =====================================================================
      SUBROUTINE DSKEWSTEVX( JOBZ, RANGE, N, E, VL, VU, IL, IU,
     $                   ABSTOL,
     $                   M, W, Z, LDZ, WORK, IWORK, IFAIL, INFO )
      IMPLICIT NONE
*
...
*     .. External Functions ..
      LOGICAL            LSAME
      DOUBLE PRECISION   DLAMCH, DLANSKEWST
      EXTERNAL           LSAME, DLAMCH, DLANSKEWST
*     ..
*     .. External Subroutines ..
      EXTERNAL           DCOPY, DSCAL, DSKEWSTEBZ, DSKEWSTEIN,
     $                   DSKEWSTEQR DSWAP, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*     call DSKEWSTEQR.  If this fails for some eigenvalue, then
*     try DSKEWSTEBZ.
*
      TEST = .FALSE.
      IF( INDEIG ) THEN
...
*      IF( ( ALLEIG .OR. TEST ) .AND. ( ABSTOL.LE.ZERO ) ) THEN
      IF( .FALSE. ) THEN
         ...
         INDWRK = N + 1
         CALL DSKEWSTEQR( 'I', N, W, Z, LDZ, WORK( INDWRK ),
     $                INFO )
         IF( INFO.EQ.0 ) THEN
            DO 10 I = 1, N
               IFAIL( I ) = 0
...
*
      ...
      INDISP = 1 + N
      INDIWO = INDISP + N
      CALL DSKEWSTEBZ( RANGE, ORDER, N, VLL, VUU, IL, IU, ABSTOL, E,
     $             M,
     $             NSPLIT, W, IWORK( 1 ), IWORK( INDISP ),
     $             WORK( INDWRK ), IWORK( INDIWO ), INFO )
*
      IF( WANTZ ) THEN
         CALL DSKEWSTEIN( N, E, M, W, IWORK( 1 ), IWORK( INDISP ),
     $                Z, LDZ, WORK( INDWRK ), IWORK( INDIWO ),
     $                IFAIL, INFO )
      END IF
*
