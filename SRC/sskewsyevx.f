*> \brief <b> SSKEWSYEVX computes the eigenvalues and, optionally, the left and/or right eigenvectors for SKEWSY matrices</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> Download SSKEWSYEVX + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/sskewsyevx.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/sskewsyevx.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/sskewsyevx.f">
*> [TXT]</a>
*
*  Definition:
*  ===========
*
*       SUBROUTINE SSKEWSYEVX( JOBZ, RANGE, UPLO, N, A, LDA, VL, VU, IL, IU,
...
*> \verbatim
*>
*> SSKEWSYEVX computes selected eigenvalues and, optionally, eigenvectors
...
*> SSKEWSYEV, the output layout according to VL/VU and IL/IU abides the
*> following rules:
*>
*> If RANGE = 'A', the same as layout of SSKEWSYEV.
*>
...
*> purely imaginary values, and are organized in the same layout as SSKEWSYEV.
*>
...
*>          where NB is the max of the blocksize for SSKEWSYTRD and SORMTR
*>          returned by ILAENV.
*>
...
*> \verbatim
*>
*>  Derived from subroutine dsyevx.
*>
*> \endverbatim
*
*> \par Contributors:
*  ==================
*>
*>    Shuo Zheng, China, Nov 2025 \n
*>
*  =====================================================================
      SUBROUTINE SSKEWSYEVX( JOBZ, RANGE, UPLO, N, A, LDA, VL, VU,
     $                   IL, IU,
     $                   ABSTOL, M, W, Z, LDZ, WORK, LWORK, IWORK,
     $                   IFAIL, INFO )
      IMPLICIT NONE
...
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ILAENV
      REAL               SLAMCH, SLANSKEWSY, SROUNDUP_LWORK
      EXTERNAL           LSAME, ILAENV, SLAMCH,
     $                   SLANSKEWSY, SROUNDUP_LWORK
*     ..
*     .. External Subroutines ..
      EXTERNAL           SCOPY, SLACPY, SORGTR, SORMTR, SSCAL,
     $                   SSKEWSTEBZ,
     $                   SSKEWSTEIN, SSKEWSTEQR, SSWAP, SSKEWSYTRD,
     $                   XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.EQ.0 ) THEN
         ...
         ELSE
            LWKMIN = 8*N
            NB = ILAENV( 1, 'SSKEWSYTRD', UPLO, N, -1, -1, -1 )
...
*
      ...
      IF( VALEIG ) THEN
         ...
         VUU = VU
      END IF
      ANRM = SLANSKEWSY( 'M', UPLO, N, A, LDA, WORK )
   ...
   20       CONTINUE
     ...
     $      ABSTLL = ABSTOL*SIGMA
      ...
      END IF
*
*     Call SSKEWSYTRD to reduce skew-symmetric matrix to tridiagonal form.
*
      INDTAU = 1
      INDE = INDTAU + N
      INDWRK = INDE + N
      LLWORK = LWORK - INDWRK + 1
      CALL SSKEWSYTRD( UPLO, N, A, LDA, WORK( INDE ),
     $             WORK( INDTAU ), WORK( INDWRK ), LLWORK, IINFO )
      IF(.NOT.LOWER)
     $   CALL SSCAL(N-1, -ONE, WORK( INDE ), 1)
*
*     If all eigenvalues are desired and ABSTOL is less than or equal to
*     zero, then call SORGTR and SSKEWSTEQR.  If this fails for some eigenvalue,
*     then try SSKEWSTEBZ.
*
      TEST = .FALSE.
      IF( INDEIG ) THEN
...
*      IF( ( ALLEIG .OR. TEST ) .AND. ( ABSTOL.LE.ZERO ) ) THEN
     ...
     $                   WORK( INDWRK ), LLWORK, IINFO )
         ...
         CALL SSKEWSTEQR( JOBZ, N, WORK( INDEE ), Z, LDZ,
     $                WORK( INDWRK ), INFO )
         IF( INFO.EQ.0 ) THEN
            DO 30 I = 1, N
               IFAIL( I ) = 0
...
*
      ...
      END IF
*
*     Otherwise, call SSKEWSTEBZ and, if eigenvectors are desired, SSKEWSTEIN.
*
      IF( WANTZ ) THEN
         ORDER = 'B'
      ELSE
      ...
      INDISP = INDIBL + N
      INDIWO = INDISP + N
      CALL SSKEWSTEBZ( RANGE, ORDER, N, VLL, VUU, IL, IU, ABSTLL,
     $             WORK( INDE ), M, NSPLIT, W, IWORK( INDIBL ),
     $             IWORK( INDISP ), WORK( INDWRK ),
     $             IWORK( INDIWO ), INFO )
*
      IF( WANTZ ) THEN
         CALL SSKEWSTEIN( N, WORK( INDE ), M, W, IWORK( INDIBL ),
     $                IWORK( INDISP ), Z, LDZ, WORK( INDWRK ), 
     $                IWORK( INDIWO ), IFAIL, INFO )
*
*        Apply orthogonal matrix used in reduction to tridiagonal
*        form to eigenvectors returned by SSKEWSTEIN.
*
         INDWKN = INDE
