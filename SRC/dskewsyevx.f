*> \brief <b> DSKEWSYEVX computes the eigenvalues and, optionally, the left and/or right eigenvectors for SKEWSY matrices</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> Download DSKEWSYEVX + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dskewsyevx.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dskewsyevx.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dskewsyevx.f">
*> [TXT]</a>
*
*  Definition:
*  ===========
*
*       SUBROUTINE DSKEWSYEVX( JOBZ, RANGE, UPLO, N, A, LDA, VL, VU, IL, IU,
...
*> \verbatim
*>
*> DSKEWSYEVX computes selected eigenvalues and, optionally, eigenvectors
...
*> DSKEWSYEV, the output layout according to VL/VU and IL/IU abides the
*> following rules:
*>
*> If RANGE = 'A', the same as layout of DSKEWSYEV.
*>
...
*> purely imaginary values, and are organized in the same layout as DSKEWSYEV.
*>
...
*>          where NB is the max of the blocksize for DSKEWSYTRD and DORMTR
*>          returned by ILAENV.
*>
...
*> \verbatim
*>
*>  Derived from subroutine ssyevx.
*>
*> \endverbatim
*
*> \par Contributors:
...
*>
*  =====================================================================
      SUBROUTINE DSKEWSYEVX( JOBZ, RANGE, UPLO, N, A, LDA, VL, VU,
     $                   IL, IU,
     $                   ABSTOL, M, W, Z, LDZ, WORK, LWORK, IWORK,
     $                   IFAIL, INFO )
      IMPLICIT NONE
...
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ILAENV
      DOUBLE PRECISION   DLAMCH, DLANSKEWSY
...
*     .. External Subroutines ..
     ...
     $                   DSKEWSTEBZ,
     $                   DSKEWSTEIN, DSKEWSTEQR, DSWAP, DSKEWSYTRD,
     $                   XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.EQ.0 ) THEN
         ...
         ELSE
            LWKMIN = 8*N
            NB = ILAENV( 1, 'DSKEWSYTRD', UPLO, N, -1, -1, -1 )
...
*
      ...
      IF( VALEIG ) THEN
         ...
         VUU = VU
      END IF
      ANRM = DLANSKEWSY( 'M', UPLO, N, A, LDA, WORK )
   ...
   20       CONTINUE
     ...
     $      ABSTLL = ABSTOL*SIGMA
      ...
      END IF
*
*     Call DSKEWSYTRD to reduce skew-symmetric matrix to tridiagonal form.
*
      INDTAU = 1
      INDE = INDTAU + N
      INDWRK = INDE + N
      LLWORK = LWORK - INDWRK + 1
      CALL DSKEWSYTRD( UPLO, N, A, LDA, WORK( INDE ),
     $             WORK( INDTAU ), WORK( INDWRK ), LLWORK, IINFO )
      IF(.NOT.LOWER)
     $   CALL DSCAL(N-1, -ONE, WORK( INDE ), 1)
*
*     If all eigenvalues are desired and ABSTOL is less than or equal to
*     zero, then call DORGTR and DSKEWSTEQR.  If this fails for some eigenvalue,
*     then try DSKEWSTEBZ.
*
      TEST = .FALSE.
      IF( INDEIG ) THEN
...
*      IF( ( ALLEIG .OR. TEST ) .AND. ( ABSTOL.LE.ZERO ) ) THEN
     ...
     $                   WORK( INDWRK ), LLWORK, IINFO )
         ...
         CALL DSKEWSTEQR( JOBZ, N, WORK( INDEE ), Z, LDZ,
     $                WORK( INDWRK ), INFO )
         IF( INFO.EQ.0 ) THEN
            DO 30 I = 1, N
               IFAIL( I ) = 0
...
*
      ...
      END IF
*
*     Otherwise, call DSKEWSTEBZ and, if eigenvectors are desired, DSKEWSTEIN.
*
      IF( WANTZ ) THEN
         ORDER = 'B'
      ELSE
      ...
      INDISP = INDIBL + N
      INDIWO = INDISP + N
      CALL DSKEWSTEBZ( RANGE, ORDER, N, VLL, VUU, IL, IU, ABSTLL,
     $             WORK( INDE ), M, NSPLIT, W, IWORK( INDIBL ),
     $             IWORK( INDISP ), WORK( INDWRK ),
     $             IWORK( INDIWO ), INFO )
*
      IF( WANTZ ) THEN
         CALL DSKEWSTEIN( N, WORK( INDE ), M, W, IWORK( INDIBL ),
     $                IWORK( INDISP ), Z, LDZ, WORK( INDWRK ), 
     $                IWORK( INDIWO ), IFAIL, INFO )
*
*        Apply orthogonal matrix used in reduction to tridiagonal
*        form to eigenvectors returned by DSKEWSTEIN.
*
         INDWKN = INDE
