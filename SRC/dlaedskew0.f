*> \brief \b DLAEDSKEW0 used by DSKEWSTEDC. Computes all eigenvalues and corresponding eigenvectors of an unreduced skew-symmetric tridiagonal matrix using the divide and conquer method.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> Download DLAEDSKEW0 + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dlaedskew0.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dlaedskew0.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dlaedskew0.f">
*> [TXT]</a>
*
*  Definition:
*  ===========
*
*       SUBROUTINE DLAEDSKEW0( ICOMPQ, QSIZ, N, E, Q, LDQ, QSTORE, LDQS,
*                          WORK, IWORK, INFO )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> DLAEDSKEW0 computes all eigenvalues and corresponding eigenvectors of a
...
*> \verbatim
*>
*>  Derived from subroutine dlaed0.
*>
*> \endverbatim
*
*> \par Contributors:
...
*>
*  =====================================================================
      SUBROUTINE DLAEDSKEW0( ICOMPQ, QSIZ, N, E, Q, LDQ, QSTORE,
...
*
     ...
     $   RETURN
      IF( ICOMPQ.EQ.0 ) THEN
         CALL DSKEWSTEQR( 'N', N, E, Q, LDQ, WORK, INFO )
         RETURN
      END IF
      IF( ICOMPQ.EQ.1 ) THEN
      ...
      END IF
*
      SMLSIZ = ILAENV( 9, 'DLAEDSKEW0', ' ', 0, 0, 0, 0 )
      NSVD = N/2
      SQRE = MOD(N, 2)
      IDXQR = N*N+1
      IDXD = IDXQR+2*N-4
*
      IF( NSVD.LE.SMLSIZ ) THEN
         CALL DSKEWSTEQR( 'I', N, E, Q, LDQ, WORK, INFO )
         GO TO 60
      END IF
*
...
*
         ...
         NRF = IC + 1
         SQREI = 1
         CALL DSKEWSTEQR( 'I', 2*NL+1, E( 2*NLF-1 ),
     ...
     $                WORK( IDXQR ), INFO )
         ...
         END IF
*
*        Reorganize the output of DSKEWSTEQR into DLASD1's layout
*
         DO J = 1, NL
...
*
   ...
   10    CONTINUE
         ...
         END IF
         NRP1 = NR + SQREI
         CALL DSKEWSTEQR( 'I', 2*NR+SQREI, E( 2*NRF-1 ),
...
*
     ...
     $             Q(NSVD+1, NSVD+1), LDQ )
*
*     Reorganize the output of DLASD1 into DSKEWSTEQR's layout
*
      DO J = 1, NSVD
