*> \brief \b SKYTF2 computes the factorization of a real symmetric indefinite matrix, using the diagonal pivoting method (unblocked algorithm).
*
*  =========== DOCUMENTATION ===========
*
...
*                  
                  ...
                  CALL SSWAP( IMAX1-K-1, A( IMAX1, K+1 ), LDA,
     $                     A( K+1, K+1 ), 1 )

                  CALL SSCAL( IMAX1-K-1, -ONE, A( IMAX1, K+1 ),
     $                     LDA )

                  CALL SSCAL( IMAX1-K-1, -ONE, A( K+1, K+1 ),
     $                     1 )
             
                  CALL SSWAP( N-IMAX1+1, A( IMAX1, IMAX1 ), 1,
     $                     A( IMAX1, K+1 ), 1 ) 
...
*
                  ...
                  CALL SSWAP( N-K-1, A( K+1, K ), LDA, A( K+1, K+1 ),
     $                     LDA )
             
                  A( K+1, K ) = -A( K+1, K )

                  CALL SSWAP( IMAX2-K-1, A( IMAX2, K+1 ), LDA,
     $                     A( K+1, K+1 ), 1 )

                  CALL SSCAL( IMAX2-K-1, -ONE, A( IMAX2, K+1 ),
     $                     LDA )

                  CALL SSCAL( IMAX2-K-1, -ONE, A( K+1, K+1 ),
     $                     1 )
             
                  CALL SSWAP( N-IMAX2+1, A( IMAX2, IMAX2 ), 1,
     $                     A( IMAX2, K+1 ), 1 ) 
