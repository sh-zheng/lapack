...
*
     ...
     $                     A( IMAX1, IMAX1+1 ), LDA )
                  ...
                  CALL DSCAL( K-IMAX1-2, -ONE, A( IMAX1, IMAX1+1 ), LDA)
...
*
     ...
     $                     A( IMAX2, IMAX2+1 ), LDA )
                  ...
                  CALL DSCAL( K-IMAX2-2, -ONE, A( IMAX2, IMAX2+1 ), LDA)
...
*
     ...
     $               W( K, 1 ), LDW, ONE, W( K, K ), 1 )
         CALL DGEMV( 'No transpose', N-K, K-1, ONE, A( K+1, 1 ), LDA,
...
*           (Here, J is a diagonal index)
            ...
            IF( J.GT.1 ) THEN
               ...
               IF( JP.LT.0 ) THEN
                  JP = -JP
*                 (Here, J is a diagonal index)
                  CALL DSWAP( J-1, A( JP, 1 ), LDA, A( JJ+1, 1 ), LDA )
                  CALL DSWAP( J-1, A( JJ+1, 1 ), LDA, A( JJ, 1 ), LDA )
               ELSEIF( JP.GT.0 ) THEN
                  CALL DSWAP( J-1, A( JP, 1 ), LDA, A( JJ+1, 1 ), LDA )
               END IF
