...
*  ===========
*
*       SUBROUTINE DCHKSKEWST( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRESH,
...
*> \verbatim
*>
*> DCHKSKEWST  checks the skew-symmetric eigenvalue problem routines.
*>
*>    DSKEWSYTRD factors A as  U S U' , where ' means transpose,
*>    S is skew-symmetric tridiagonal, and U is orthogonal.
*>    DSKEWSYTRD can use either just the lower or just the upper triangle
*>    of A; DCHKSKEWST checks both cases.
*>    U is represented as a product of Householder
*>    transformations, whose vectors are stored in the first
*>    n-1 columns of V, and whose scale factors are in TAU.
*>
*>    DSKEWSTEQR factors S as  Z D1 Z' , where Z is the orthogonal
*>    matrix of eigenvectors and D1 is a diagonal matrix with
*>    the eigenvalues on the diagonal.  D2 is the matrix of
*>    eigenvalues computed when Z is not computed.
*>
*>    DSKEWSTEBZ computes selected eigenvalues.  WA1, WA2, and
*>    WA3 will denote eigenvalues computed to high
*>    absolute accuracy, with different range options.
*>    WR will denote eigenvalues computed to high relative
*>    accuracy.
*>
*>    DSKEWSTEIN computes Y, the eigenvectors of S, given the
*>    eigenvalues.
*>
*>    DSKEWSTEDC factors S as Z D1 Z' , where Z is the orthogonal
*>    matrix of eigenvectors and D1 is a diagonal matrix with
...
*>
*> When DCHKSKEWST is called, a number of matrix "sizes" ("n's") and a
*> number of matrix "types" are specified.  For each size ("n")
*> and each type of matrix, one matrix will be generated and used
*> to test the skew-symmetric eigenroutines.  For each matrix, a
*> number of tests will be performed:
*>
*> (1)     | A - V S V' | / ( |A| n ulp ) DSKEWSYTRD( UPLO='U', ... )
*>
*> (2)     | I - UV' | / ( n ulp )        DORGTR( UPLO='U', ... )
*>
*> (3)     | A - V S V' | / ( |A| n ulp ) DSKEWSYTRD( UPLO='L', ... )
*>
*> (4)     | I - UV' | / ( n ulp )        DORGTR( UPLO='L', ... )
*>
*> (5-8)   Same as 1-4, but for SSPTRD and SOPGTR.
*>
*> (9)     | S - Z D Z' | / ( |S| n ulp ) DSKEWSTEQR('V',...)
*>
*> (10)    | I - ZZ' | / ( n ulp )        DSKEWSTEQR('V',...)
*>
*> (11)    | D1 - D2 | / ( |D1| ulp )        DSKEWSTEQR('N',...)
*>
...
*>                                              DSKEWSTEBZ( 'A', 'E', ...)
*>
*> (18)    | WA1 - D3 | / ( |D3| ulp )          DSKEWSTEBZ( 'A', 'E', ...)
*>
...
*>                                              DSKEWSTEBZ( 'I', 'E', ...)
*>
*> (20)    | S - Y WA1 Y' | / ( |S| n ulp )  DSKEWSTEBZ, DSKEWSTEIN
*>
*> (21)    | I - Y Y' | / ( n ulp )          DSKEWSTEBZ, DSKEWSTEIN
*>
*> (22)    | S - Z D Z' | / ( |S| n ulp )    DSKEWSTEDC('I')
*>
*> (23)    | I - ZZ' | / ( n ulp )           DSKEWSTEDC('I')
*>
*> (24)    | S - Z D Z' | / ( |S| n ulp )    DSKEWSTEDC('V')
*>
*> (25)    | I - ZZ' | / ( n ulp )           DSKEWSTEDC('V')
*>
*> (26)    | D1 - D2 | / ( |D1| ulp )           DSKEWSTEDC('V') and
*>                                              DSKEWSTEDC('N')
*>
...
*>          DCHKSKEWST does nothing.  It must be at least zero.
*> \endverbatim
*>
*> \param[in] NN
*> \verbatim
...
*> \verbatim
*>          NTYPES is INTEGER
*>          The number of elements in DOTYPE.   If it is zero, DCHKSKEWST
...
*>          next call to DCHKSKEWST to continue the same random number
*>          sequence.
*> \endverbatim
*>
*> \param[in] THRESH
...
*>                             dimension( max(NN) )
*>          The diagonal of the tridiagonal matrix computed by DSKEWSYTRD.
...
*>          DSKEWSYTRD.  On exit, SD and SE contain the tridiagonal form of
*>          the matrix in A.
*> \endverbatim
*>
*> \param[out] D1
...
*>                             dimension( max(NN) )
*>          The eigenvalues of A, as computed by DSKEWSTEQR simultaneously
...
*>                             dimension( max(NN) )
*>          The eigenvalues of A, as computed by DSKEWSTEQR if Z is not
...
*>          The orthogonal matrix computed by DSKEWSYTRD + DORGTR.
*> \endverbatim
*>
*> \param[in] LDU
*> \verbatim
...
*>                             dimension( LDU, max(NN) ).
*>          The Housholder vectors computed by DSKEWSYTRD in reducing A to
*>          tridiagonal form.  The vectors computed with UPLO='U' are
*>          in the upper triangle, and the vectors computed with UPLO='L'
*>          are in the lower triangle.  (As described in DSKEWSYTRD, the
...
*>                             dimension( max(NN) )
*>          The Householder factors computed by DSKEWSYTRD in reducing A
*>          to tridiagonal form.
*> \endverbatim
*>
*> \param[out] Z
...
*>                             dimension( LDU, max(NN) ).
*>          The orthogonal matrix of eigenvectors computed by DSKEWSTEQR,
*>          SPTEQR, and SSTEIN.
*> \endverbatim
*>
*> \param[out] WORK
...
*>          -29: LWORK too small.
*>          If  DLATMR, DLATMS, DSKEWSYTRD, DORGTR, DSKEWSTEQR, DSTERF,
...
*     .. External Subroutines ..
      EXTERNAL           DCOPY, DLABAD, DLACPY, DLASET, DLASUM,
     $                   DLATMR, DLATMS, DORGTR, DSKEWSTEBZ,
     $                   DSKEWSTEIN, DSKEWSTEQR, DSKEWSTT21,
     $                   DSKEWSTT22, DSKEWSYT21, DSKEWSYTRD,
     $                   DSKEWSTEDC, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   10 CONTINUE
*
      NBLOCK = ILAENV( 1, 'DSKEWSYTRD', 'L', NMAX, -1, -1, -1 )
...
*
  100       CONTINUE
*
*           Call DSKEWSYTRD and DORGTR to compute S and U from
*           upper triangle.
*
            CALL DLACPY( 'U', N, N, A, LDA, V, LDU )
*
            NTEST = 1
            CALL DSKEWSYTRD( 'U', N, V, LDU, SE, TAU, WORK, LWORK,
     $                   IINFO )
            CALL DLASET( 'N', N, 1, ZERO, ZERO, SD, N)
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'DSKEWSYTRD(U)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*           Do tests 1 and 2
*
            CALL DSKEWSYT21( 2, 'Upper', N, 1, A, LDA, SD, SE, U, LDU,
     $                   V, LDU, TAU, WORK, RESULT( 1 ) )
            CALL DSKEWSYT21( 3, 'Upper', N, 1, A, LDA, SD, SE, U, LDU,
...
*
            NTEST = 3
            CALL DSKEWSYTRD( 'L', N, V, LDU, SE, TAU, WORK, LWORK,
     $                   IINFO )
            CALL DLASET( 'N', N, 1, ZERO, ZERO, SD, N)
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'DSKEWSYTRD(L)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*           Do tests 3 and 4
*
            CALL DSKEWSYT21( 2, 'Lower', N, 1, A, LDA, SD, SE, U, LDU,
     $                   V, LDU, TAU, WORK, RESULT( 3 ) )
            CALL DSKEWSYT21( 3, 'Lower', N, 1, A, LDA, SD, SE, U, LDU,
     $                   V, LDU, TAU, WORK, RESULT( 4 ) )
*
*           Call DSKEWSTEQR to compute D1, D2, and Z, do tests.
*
*           Compute D1 and Z
*
...
*
            NTEST = 5
            CALL DSKEWSTEQR( 'V', N, WORK, Z, LDU, WORK( N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'DSKEWSTEQR(V)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            NTEST = 7
            CALL DSKEWSTEQR( 'N', N, WORK, WORK( N+1 ), LDU,
     $                   WORK( N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'DSKEWSTEQR(N)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*           Do Tests 5 and 6
*
            CALL DSKEWSTT21( N, 1, DUMMA, SE, DUMMA, D1, Z, LDU, WORK,
     $                   RESULT( 5 ) )
*
*           Do Tests 7
*
...
*
  170       CONTINUE
            RESULT( 8 ) = TEMP1
*
*           Call DSKEWSTEBZ with different options and do tests 17-18.
*
...
*
            ...
            IF( JTYPE.EQ.21 ) THEN
               NTEST = 9
               ABSTOL = UNFL + UNFL
               CALL DSKEWSTEBZ( 'A', 'E', N, VL, VU, IL, IU, ABSTOL, SE,
     $                      M, NSPLIT, WR, IWORK( 1 ), IWORK( N+1 ),
     $                      WORK, IWORK( 2*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'DSKEWSTEBZ(A,rel)', IINFO,
     $               N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
            NTEST = 10
            ABSTOL = UNFL + UNFL
            CALL DSKEWSTEBZ( 'A', 'E', N, VL, VU, IL, IU, ABSTOL, SE, M,
     $                   NSPLIT, WA1, IWORK( 1 ), IWORK( N+1 ), WORK,
     $                   IWORK( 2*N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'DSKEWSTEBZ(A)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            ...
            END IF
*
            CALL DSKEWSTEBZ( 'I', 'E', N, VL, VU, IL, IU, ABSTOL, SE,
     $                   M2, NSPLIT, WA2, IWORK( 1 ), IWORK( N+1 ),
     $                   WORK, IWORK( 2*N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'DSKEWSTEBZ(I)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
     ...
     $                 ULP*ANORM, TWO*RTUNFL )
            ...
            END IF
*
            CALL DSKEWSTEBZ( 'V', 'E', N, VL, VU, IL, IU, ABSTOL, SE,
     $                   M3, NSPLIT, WA3, IWORK( 1 ), IWORK( N+1 ),
     $                   WORK, IWORK( 2*N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'DSKEWSTEBZ(V)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
*           Call DSKEWSTEIN to compute eigenvectors corresponding to
*           eigenvalues in WA1.  (First call DSKEWSTEBZ again, to make sure
*           it returns these eigenvalues in the correct order.)
*
            NTEST = 13
            CALL DSKEWSTEBZ( 'A', 'B', N, VL, VU, IL, IU, ABSTOL, SE, M,
     $                   NSPLIT, WA1, IWORK( 1 ), IWORK( N+1 ), WORK,
     $                   IWORK( 2*N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'DSKEWSTEBZ(A,B)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
            ...
            END IF
*
            CALL DSKEWSTEIN( N, SE, M, WA1, IWORK( 1 ), IWORK( N+1 ), Z,
     $                   LDU, WORK, IWORK( 2*N+1 ), IWORK( 3*N+1 ),
     $                   IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'DSKEWSTEIN', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*           Do tests 12 and 13
*
            CALL DSKEWSTT21( N, 1, SD, SE, DUMMA, WA1, Z, LDU, WORK,
     $                   RESULT( 12 ) )
*
*           Call DSKEWSTEDC(I) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
            CALL DCOPY( N, SD, 1, D1, 1 )
...
*
            NTEST = 14
            CALL DSKEWSTEDC( 'I', N, WORK, Z, LDU, WORK( N+1 ), LWEDC-N,
     $                   IWORK, LIWEDC, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'DSKEWSTEDC(I)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            IF( N.GT.0 )
     $         CALL DCOPY( N-1, WORK, 1, D1, 1 )
            CALL DSKEWSTT21( N, 1, SD, SE, DUMMA, D1, Z, LDU, WORK,
     $                   RESULT( 14 ) )
*
*           Call DSKEWSTEDC(V) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
            CALL DCOPY( N, SD, 1, D1, 1 )
...
*
            NTEST = 16
            CALL DSKEWSTEDC( 'V', N, WORK, Z, LDU, WORK( N+1 ), LWEDC-N,
     $                   IWORK, LIWEDC, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'DSKEWSTEDC(V)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
     ...
     $                   RESULT( 16 ) )
*
*           Call DSKEWSTEDC(N) to compute D2, do tests.
*
*           Compute D2
*
            CALL DCOPY( N, SD, 1, D2, 1 )
...
*
            NTEST = 18
            CALL DSKEWSTEDC( 'N', N, WORK, Z, LDU, WORK( N+1 ), LWEDC-N,
     $                   IWORK, LIWEDC, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'DSKEWSTEDC(N)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*     Summary
*
      CALL DLASUM( 'DSKEWST', NOUNIT, NERRS, NTESTT )
      RETURN
*
 9999 FORMAT( ' DCHKSKEWST: ', A, ' returned INFO=', I6, '.', / 9X,
     $      'N=', I6, ', JTYPE=', I6, ', ISEED=(', 3( I5, ',' ), I5,
     $      ')' )
*
 9998 FORMAT( / 1X, A3, ' -- DOUBLE PRECISION Skew-symmetric',
     $        / 'eigenvalue problem' )
 9997 FORMAT( ' Matrix types (see DCHKSKEWST for details): ' )
*
...
*
 9990 FORMAT( ' N=', I5, ', seed=', 4( I4, ',' ), ' type ', I2,
     $      ', test(', I2, ')=', G10.3 )
*
 9988 FORMAT( / 'Test performed:  see DCHKSKEWST for details.', / )
*     End of DCHKSKEWST
*
      END
