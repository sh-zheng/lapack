...
*  ===========
*
*       SUBROUTINE DDRVSKEWSG2STG( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRESH,
...
*> \verbatim
*>
*>      DDRVSKEWSG2STG checks the real skew-symmetric generalized eigenproblem
*>      drivers.
*>
*>              DSKEWSYGV computes all eigenvalues and, optionally,
*>              eigenvectors of a real skew-symmetric-definite generalized
*>              eigenproblem.
*>
*>              DSKEWSYGVX computes selected eigenvalues and, optionally,
*>              eigenvectors of a real skew-symmetric-definite generalized
*>              eigenproblem.
*>
*>              DSKEWSYGVD computes all eigenvalues and, optionally,
*>              eigenvectors of a real symmetric-definite generalized
*>              eigenproblem using a divide and conquer algorithm.
*>
*>      When DDRVSKEWSG2STG is called, a number of matrix "sizes" ("n's") and a
...
*>
*>      (1) DSKEWSYGV with ITYPE = 1 and UPLO ='U':
*>
*>              | A Z - B Z D | / ( |A| |Z| n ulp )
*>              | D - D2 | / ( |D| ulp )   where D is computed by
*>                                         DSKEWSYGV and  D2 is computed by
*>                                         DSKEWSYGV_2STAGE. This test is
*>                                         only performed for DSKEWSYGV
*>
...
*>
*>      (7) DSKEWSYGV with ITYPE = 2 and UPLO ='U':
*>
*>              | A B Z - Z D | / ( |A| |Z| n ulp )
*>
...
*>
*>      (11) DSKEWSYGV with ITYPE = 3 and UPLO ='U':
*>
*>              | B A Z - Z D | / ( |A| |Z| n ulp )
*>
...
*>
*>      DSKEWSYGVD, SSPGVD and SSBGVD performed the same 14 tests.
*>
*>      DSKEWSYGVX, SSPGVX and SSBGVX performed the above 14 tests with
...
*>          DDRVSKEWSG2STG does nothing.  It must be at least zero.
*>          Not modified.
*>
...
*>
*>  NTYPES  INTEGER
*>          The number of elements in DOTYPE.   If it is zero, DDRVSKEWSG2STG
...
*>          next call to DDRVSKEWSG2STG to continue the same random number
*>          sequence.
*>          Modified.
*>
...
*>          -21: NWORK too small.
*>          -23: LIWORK too small.
*>          If  DLATMR, DLATMS, DSKEWSYGV, SSPGV, SSBGV, DSKEWSYGVD, SSPGVD,
*>              SSBGVD, DSKEWSYGVX, SSPGVX or SSBGVX returns an error code,
...
*
*  =====================================================================
      SUBROUTINE DDRVSKEWSG2STG( NSIZES, NN, NTYPES, DOTYPE, ISEED,
...
*     .. External Subroutines ..
     ...
     $                   DLATMS, DSKEWSYGV, DSKEWSYGVX, DSKEWSYGVD,
     $                   DSKEWSGT01, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL XERBLA( 'DDRVSKEWSG2STG', -INFO )
         RETURN
      END IF
*
...
*
            ...
            END IF
*
*           3) Call DSKEWSYGV, SSPGV, SSBGV, DSKEWSYGVD, SSPGVD, SSBGVD,
*              DSKEWSYGVX, SSPGVX, and SSBGVX, do tests.
*
...
*
                  CALL DSKEWSYGV( IBTYPE, 'V', UPLO, N, Z, LDZ, BB,
     $                        LDB, D, WORK, NWORK, IINFO )
*
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'DSKEWSYGV(V,' // UPLO
...
*                 Do Test
*
                  CALL DSKEWSGT01( IBTYPE, UPLO, N, N, A, LDA, B, LDB,
...
*
                  ...
                  CALL DLACPY( UPLO, N, N, B, LDB, BB, LDB )
*
                  CALL DSKEWSYGVD( IBTYPE, 'V', UPLO, N, Z, LDZ, BB,
     $                         LDB, D, WORK, NWORK, IWORK, LIWORK,
     $                         IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'DSKEWSYGVD(V,' // UPLO
     $                  // ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
                  ...
                  CALL DLACPY( UPLO, N, N, B, LDB, BB, LDB )
*
                  CALL DSKEWSYGVX( IBTYPE, 'V', 'A', UPLO, N, AB, LDA,
     $                         BB, LDB, VL, VU, IL, IU, ABSTOL, M, D,
     $                         Z, LDZ, WORK, NWORK, IWORK( N+1 ),
     $                         IWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'DSKEWSYGVX(V,A' // UPLO
     $                  // ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
                  VL = ZERO
                  VU = ANORM
                  CALL DSKEWSYGVX( IBTYPE, 'V', 'V', UPLO, N, AB, LDA,
     $                         BB, LDB, VL, VU, IL, IU, ABSTOL, M, D,
     $                         Z, LDZ, WORK, NWORK, IWORK( N+1 ),
     $                         IWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'DSKEWSYGVX(V,V,' //
...
*                 Do Test
*
                  CALL DSKEWSGT01( IBTYPE, UPLO, N, M, A, LDA, B, LDB,
     $                         Z, LDZ, D, WORK, RESULT( NTEST ) )
*
                  NTEST = NTEST + 1
*
                  ...
                  CALL DLACPY( UPLO, N, N, B, LDB, BB, LDB )
*
                  CALL DSKEWSYGVX( IBTYPE, 'V', 'I', UPLO, N, AB, LDA,
     $                         BB, LDB, VL, VU, IL, IU, ABSTOL, M, D,
     $                         Z, LDZ, WORK, NWORK, IWORK( N+1 ),
     $                         IWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'DSKEWSYGVX(V,I,' //
     $                  UPLO // ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
      RETURN
*
*     End of DDRVSKEWSG2STG
*
 9999 FORMAT( ' DDRVSKEWSG2STG: ', A, ' returned INFO=', I6, '.', / 9X,
