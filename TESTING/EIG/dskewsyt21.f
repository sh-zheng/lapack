...
*  ===========
*
*       SUBROUTINE DSKEWSYT21( ITYPE, UPLO, N, KBAND, A, LDA, D, E, U, LDU, V,
*                          LDV, TAU, WORK, RESULT )
*
*       .. Scalar Arguments ..
*       CHARACTER          UPLO
...
*> \verbatim
*>
*> DSKEWSYT21 generally checks a decomposition of the form
*>
*>    A = U S U**T
*>
...
*> \verbatim
*>          N is INTEGER
*>          The size of the matrix.  If it is zero, DSKEWSYT21 does nothing.
...
*
*  =====================================================================
      SUBROUTINE DSKEWSYT21( ITYPE, UPLO, N, KBAND, A, LDA, D, E, U,
     $                   LDU, V, LDV, TAU, WORK, RESULT )
      IMPLICIT NONE
*
*  -- LAPACK test routine --
...
*     .. External Functions ..
      LOGICAL            LSAME
      DOUBLE PRECISION   DLAMCH, DLANGE, DLANSKEWSY
      EXTERNAL           LSAME, DLAMCH, DLANGE, DLANSKEWSY
*     ..
*     .. External Subroutines ..
      EXTERNAL           DGEMM, DLACPY, DLARFYSKEW, DLASET, DORM2L,
     $                   DORM2R, DSKEWSYR2
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( ITYPE.EQ.3 ) THEN
         ANORM = ONE
      ELSE
         ANORM = MAX( DLANSKEWSY( '1', CUPLO, N, A, LDA, WORK ),
     $                            UNFL )
      END IF
*
...
*
         IF( N.GT.1 .AND. KBAND.EQ.1 ) THEN
            DO 20 J = 1, N - 1
               CALL DSKEWSYR2( CUPLO, N, -E( J ), U( 1, J ), 1,
     $                     U( 1, J+1 ), 1, WORK, N )
   20       CONTINUE
         END IF
         WNORM = DLANSKEWSY( '1', CUPLO, N, WORK, N,
     $                       WORK( N**2+1 ) )
*
...
*
               VSAVE = V( J+1, J )
               V( J+1, J ) = ONE
               CALL DLARFYSKEW( 'L', N-J, V( J+1, J ), 1, TAU( J ),
...
*
               VSAVE = V( J, J+1 )
               V( J, J+1 ) = ONE
               CALL DLARFYSKEW( 'U', J, V( 1, J+1 ), 1, TAU( J ),
     $                      WORK, N, WORK( N**2+1 ) )
               V( J, J+1 ) = VSAVE
               WORK( ( N+1 )*J+1 ) = ZERO
   60       CONTINUE
