...
*  ===========
*
*       SUBROUTINE SCHKSKEWST( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRESH,
...
*> \verbatim
*>
*> SCHKSKEWST  checks the skew-symmetric eigenvalue problem routines.
*>
*>    SSKEWSYTRD factors A as  U S U' , where ' means transpose,
*>    S is skew-symmetric tridiagonal, and U is orthogonal.
*>    SSKEWSYTRD can use either just the lower or just the upper triangle
*>    of A; SCHKSKEWST checks both cases.
*>    U is represented as a product of Householder
*>    transformations, whose vectors are stored in the first
*>    n-1 columns of V, and whose scale factors are in TAU.
*>
*>    SSKEWSTEQR factors S as  Z D1 Z' , where Z is the orthogonal
*>    matrix of eigenvectors and D1 is a diagonal matrix with
*>    the eigenvalues on the diagonal.  D2 is the matrix of
*>    eigenvalues computed when Z is not computed.
*>
*>    SSKEWSTEBZ computes selected eigenvalues.  WA1, WA2, and
*>    WA3 will denote eigenvalues computed to high
*>    absolute accuracy, with different range options.
*>    WR will denote eigenvalues computed to high relative
*>    accuracy.
*>
*>    SSKEWSTEIN computes Y, the eigenvectors of S, given the
*>    eigenvalues.
*>
*>    SSKEWSTEDC factors S as Z D1 Z' , where Z is the orthogonal
*>    matrix of eigenvectors and D1 is a diagonal matrix with
...
*>
*> When SCHKSKEWST is called, a number of matrix "sizes" ("n's") and a
*> number of matrix "types" are specified.  For each size ("n")
*> and each type of matrix, one matrix will be generated and used
*> to test the skew-symmetric eigenroutines.  For each matrix, a
*> number of tests will be performed:
*>
*> (1)     | A - V S V' | / ( |A| n ulp ) SSKEWSYTRD( UPLO='U', ... )
*>
*> (2)     | I - UV' | / ( n ulp )        SORGTR( UPLO='U', ... )
*>
*> (3)     | A - V S V' | / ( |A| n ulp ) SSKEWSYTRD( UPLO='L', ... )
*>
*> (4)     | I - UV' | / ( n ulp )        SORGTR( UPLO='L', ... )
*>
*> (5-8)   Same as 1-4, but for SSPTRD and SOPGTR.
*>
*> (9)     | S - Z D Z' | / ( |S| n ulp ) SSKEWSTEQR('V',...)
*>
*> (10)    | I - ZZ' | / ( n ulp )        SSKEWSTEQR('V',...)
*>
*> (11)    | D1 - D2 | / ( |D1| ulp )        SSKEWSTEQR('N',...)
*>
...
*>                                              SSKEWSTEBZ( 'A', 'E', ...)
*>
*> (18)    | WA1 - D3 | / ( |D3| ulp )          SSKEWSTEBZ( 'A', 'E', ...)
*>
...
*>                                              SSKEWSTEBZ( 'I', 'E', ...)
*>
*> (20)    | S - Y WA1 Y' | / ( |S| n ulp )  SSKEWSTEBZ, SSKEWSTEIN
*>
*> (21)    | I - Y Y' | / ( n ulp )          SSKEWSTEBZ, SSKEWSTEIN
*>
*> (22)    | S - Z D Z' | / ( |S| n ulp )    SSKEWSTEDC('I')
*>
*> (23)    | I - ZZ' | / ( n ulp )           SSKEWSTEDC('I')
*>
*> (24)    | S - Z D Z' | / ( |S| n ulp )    SSKEWSTEDC('V')
*>
*> (25)    | I - ZZ' | / ( n ulp )           SSKEWSTEDC('V')
*>
*> (26)    | D1 - D2 | / ( |D1| ulp )           SSKEWSTEDC('V') and
*>                                              SSKEWSTEDC('N')
*>
...
*>          SCHKSKEWST does nothing.  It must be at least zero.
*> \endverbatim
*>
*> \param[in] NN
*> \verbatim
...
*> \verbatim
*>          NTYPES is INTEGER
*>          The number of elements in DOTYPE.   If it is zero, SCHKSKEWST
...
*>          next call to SCHKSKEWST to continue the same random number
*>          sequence.
*> \endverbatim
*>
*> \param[in] THRESH
...
*>          SD is REAL array of
*>                             dimension( max(NN) )
*>          The diagonal of the tridiagonal matrix computed by SSKEWSYTRD.
...
*>          SSKEWSYTRD.  On exit, SD and SE contain the tridiagonal form of
*>          the matrix in A.
*> \endverbatim
*>
*> \param[out] D1
...
*>          D1 is REAL array of
*>                             dimension( max(NN) )
*>          The eigenvalues of A, as computed by SSKEWSTEQR simultaneously
...
*>          D2 is REAL array of
*>                             dimension( max(NN) )
*>          The eigenvalues of A, as computed by SSKEWSTEQR if Z is not
...
*>          The orthogonal matrix computed by SSKEWSYTRD + SORGTR.
*> \endverbatim
*>
*> \param[in] LDU
*> \verbatim
...
*>          V is REAL array of
*>                             dimension( LDU, max(NN) ).
*>          The Housholder vectors computed by SSKEWSYTRD in reducing A to
*>          tridiagonal form.  The vectors computed with UPLO='U' are
*>          in the upper triangle, and the vectors computed with UPLO='L'
*>          are in the lower triangle.  (As described in SSKEWSYTRD, the
...
*>                             dimension( max(NN) )
*>          The Householder factors computed by SSKEWSYTRD in reducing A
*>          to tridiagonal form.
*> \endverbatim
*>
*> \param[out] Z
...
*>          Z is REAL array of
*>                             dimension( LDU, max(NN) ).
*>          The orthogonal matrix of eigenvectors computed by SSKEWSTEQR,
*>          SPTEQR, and SSTEIN.
*> \endverbatim
*>
*> \param[out] WORK
...
*>          -29: LWORK too small.
*>          If  SLATMR, SLATMS, SSKEWSYTRD, SORGTR, SSKEWSTEQR, SSTERF,
...
*     .. External Subroutines ..
      EXTERNAL           SCOPY, SLABAD, SLACPY, SLASET, SLASUM,
     $                   SLATMR, SLATMS, SORGTR, SSKEWSTEBZ,
     $                   SSKEWSTEIN, SSKEWSTEQR, SSKEWSTT21,
     $                   SSKEWSTT22, SSKEWSYT21, SSKEWSYTRD,
     $                   SSKEWSTEDC, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   10 CONTINUE
*
      NBLOCK = ILAENV( 1, 'SSKEWSYTRD', 'L', NMAX, -1, -1, -1 )
...
*
               CALL SLASET( 'N', N, N, ZERO, ZERO, A, LDA )
               DO 90 I = 2, N
                  A( I-1, I ) = ONE
                  A( I, I-1 ) = -ONE
   90          CONTINUE
...
*
  100       CONTINUE
*
*           Call SSKEWSYTRD and SORGTR to compute S and U from
*           upper triangle.
*
            CALL SLACPY( 'U', N, N, A, LDA, V, LDU )
*
            NTEST = 1
            CALL SSKEWSYTRD( 'U', N, V, LDU, SE, TAU, WORK, LWORK,
     $                   IINFO )
            CALL SLASET( 'N', N, 1, ZERO, ZERO, SD, N)
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'SSKEWSYTRD(U)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*           Do tests 1 and 2
*
            CALL SSKEWSYT21( 2, 'Upper', N, 1, A, LDA, SD, SE, U, LDU,
     $                   V, LDU, TAU, WORK, RESULT( 1 ) )
            CALL SSKEWSYT21( 3, 'Upper', N, 1, A, LDA, SD, SE, U, LDU,
     $                   V, LDU, TAU, WORK, RESULT( 2 ) )
*
*           Call SSKEWSYTRD and SORGTR to compute S and U from
...
*
            NTEST = 3
            CALL SSKEWSYTRD( 'L', N, V, LDU, SE, TAU, WORK, LWORK,
     $                   IINFO )
            CALL SLASET( 'N', N, 1, ZERO, ZERO, SD, N)
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'SSKEWSYTRD(L)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*           Do tests 3 and 4
*
            CALL SSKEWSYT21( 2, 'Lower', N, 1, A, LDA, SD, SE, U, LDU,
     $                   V, LDU, TAU, WORK, RESULT( 3 ) )
            CALL SSKEWSYT21( 3, 'Lower', N, 1, A, LDA, SD, SE, U, LDU,
     $                   V, LDU, TAU, WORK, RESULT( 4 ) )
*
*           Call SSKEWSTEQR to compute D1, D2, and Z, do tests.
*
*           Compute D1 and Z
*
...
*
            NTEST = 5
            CALL SSKEWSTEQR( 'V', N, WORK, Z, LDU, WORK( N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEQR(V)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            NTEST = 7
            CALL SSKEWSTEQR( 'N', N, WORK, WORK( N+1 ), LDU,
     $                   WORK( N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEQR(N)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*           Do Tests 5 and 6
*
            CALL SSKEWSTT21( N, 1, DUMMA, SE, DUMMA, D1, Z, LDU, WORK,
     $                   RESULT( 5 ) )
*
*           Do Tests 7
*
...
*
  170       CONTINUE
            RESULT( 8 ) = TEMP1
*
*           Call SSKEWSTEBZ with different options and do tests 17-18.
*
...
*
            ...
            IF( JTYPE.EQ.21 ) THEN
               NTEST = 9
               ABSTOL = UNFL + UNFL
               CALL SSKEWSTEBZ( 'A', 'E', N, VL, VU, IL, IU, ABSTOL, SE,
     $                      M, NSPLIT, WR, IWORK( 1 ), IWORK( N+1 ),
     $                      WORK, IWORK( 2*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEBZ(A,rel)', IINFO,
     $               N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
            NTEST = 10
            ABSTOL = UNFL + UNFL
            CALL SSKEWSTEBZ( 'A', 'E', N, VL, VU, IL, IU, ABSTOL, SE, M,
     $                   NSPLIT, WA1, IWORK( 1 ), IWORK( N+1 ), WORK,
     $                   IWORK( 2*N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEBZ(A)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            ...
            END IF
*
            CALL SSKEWSTEBZ( 'I', 'E', N, VL, VU, IL, IU, ABSTOL, SE,
     $                   M2, NSPLIT, WA2, IWORK( 1 ), IWORK( N+1 ),
     $                   WORK, IWORK( 2*N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEBZ(I)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
     ...
     $                 ULP*ANORM, TWO*RTUNFL )
            ...
            END IF
*
            CALL SSKEWSTEBZ( 'V', 'E', N, VL, VU, IL, IU, ABSTOL, SE,
     $                   M3, NSPLIT, WA3, IWORK( 1 ), IWORK( N+1 ),
     $                   WORK, IWORK( 2*N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEBZ(V)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
*           Call SSKEWSTEIN to compute eigenvectors corresponding to
*           eigenvalues in WA1.  (First call SSKEWSTEBZ again, to make sure
*           it returns these eigenvalues in the correct order.)
*
            NTEST = 13
            CALL SSKEWSTEBZ( 'A', 'B', N, VL, VU, IL, IU, ABSTOL, SE, M,
     $                   NSPLIT, WA1, IWORK( 1 ), IWORK( N+1 ), WORK,
     $                   IWORK( 2*N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEBZ(A,B)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
            ...
            END IF
*
            CALL SSKEWSTEIN( N, SE, M, WA1, IWORK( 1 ), IWORK( N+1 ), Z,
     $                   LDU, WORK, IWORK( 2*N+1 ), IWORK( 3*N+1 ),
     $                   IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEIN', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*           Do tests 12 and 13
*
            CALL SSKEWSTT21( N, 1, SD, SE, DUMMA, WA1, Z, LDU, WORK,
     $                   RESULT( 12 ) )
*
*           Call SSKEWSTEDC(I) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
            CALL SCOPY( N, SD, 1, D1, 1 )
...
*
            NTEST = 14
            CALL SSKEWSTEDC( 'I', N, WORK, Z, LDU, WORK( N+1 ), LWEDC-N,
     $                   IWORK, LIWEDC, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEDC(I)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            IF( N.GT.0 )
     $         CALL SCOPY( N-1, WORK, 1, D1, 1 )
            CALL SSKEWSTT21( N, 1, SD, SE, DUMMA, D1, Z, LDU, WORK,
     $                   RESULT( 14 ) )
*
*           Call SSKEWSTEDC(V) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
            CALL SCOPY( N, SD, 1, D1, 1 )
...
*
            NTEST = 16
            CALL SSKEWSTEDC( 'V', N, WORK, Z, LDU, WORK( N+1 ), LWEDC-N,
     $                   IWORK, LIWEDC, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEDC(V)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
     ...
     $                   RESULT( 16 ) )
*
*           Call SSKEWSTEDC(N) to compute D2, do tests.
*
*           Compute D2
*
            CALL SCOPY( N, SD, 1, D2, 1 )
...
*
            NTEST = 18
            CALL SSKEWSTEDC( 'N', N, WORK, Z, LDU, WORK( N+1 ), LWEDC-N,
     $                   IWORK, LIWEDC, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEDC(N)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*     Summary
*
      CALL SLASUM( 'SSKEWST', NOUNIT, NERRS, NTESTT )
      RETURN
*
 9999 FORMAT( ' SCHKSKEWST: ', A, ' returned INFO=', I6, '.', / 9X,
     $      'N=', I6, ', JTYPE=', I6, ', ISEED=(', 3( I5, ',' ), I5,
     $      ')' )
*
 9998 FORMAT( / 1X, A3, ' -- Real Skew-symmetric eigenvalue problem' )
 9997 FORMAT( ' Matrix types (see SCHKSKEWST for details): ' )
*
...
*
 9990 FORMAT( ' N=', I5, ', seed=', 4( I4, ',' ), ' type ', I2,
     $      ', test(', I2, ')=', G10.3 )
*
 9988 FORMAT( / 'Test performed:  see SCHKSKEWST for details.', / )
*     End of SCHKSKEWST
*
      END
