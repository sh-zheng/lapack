...
*  ===========
*
*       SUBROUTINE SDRVSKEWST( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRESH,
...
*> \verbatim
*>
*>      SDRVSKEWST  checks the skew-symmetric eigenvalue problem drivers.
*>
*>              SSKEWSTEV computes all eigenvalues and, optionally,
*>              eigenvectors of a real skew-symmetric tridiagonal matrix.
*>
*>              SSKEWSTEVX computes selected eigenvalues and, optionally,
*>              eigenvectors of a real skew-symmetric tridiagonal matrix.
*>
*>              SSKEWSYEV computes all eigenvalues and, optionally,
*>              eigenvectors of a real skew-symmetric matrix.
*>
*>              SSKEWSYEVX computes selected eigenvalues and, optionally,
*>              eigenvectors of a real skew-symmetric matrix.
*>
*>              SSKEWSYEVD computes all eigenvalues and, optionally,
*>              eigenvectors of a real symmetric matrix using
*>              a divide and conquer algorithm.
*>
*>      When SDRVSKEWST is called, a number of matrix "sizes" ("n's") and a
...
*>          SDRVSKEWST does nothing.  It must be at least zero.
*>          Not modified.
*>
...
*>
*>  NTYPES  INTEGER
*>          The number of elements in DOTYPE.   If it is zero, SDRVSKEWST
...
*>          next call to SDRVSKEWST to continue the same random number
*>          sequence.
*>          Modified.
*>
*>  THRESH  REAL
...
*>          The eigenvalues as computed by SSKEWSTEV('N', ... )
...
*>    1= | A - U S U' | / ( |A| n ulp )         SSKEWSTEV('V', ... )
*>    2= | I - U U' | / ( n ulp )               SSKEWSTEV('V', ... )
*>    3= |D(with Z) - D(w/o Z)| / (|D| ulp)     SSKEWSTEV('N', ... )
*>    4= | A - U S U' | / ( |A| n ulp )         SSKEWSTEVX('V','A', ... )
*>    5= | I - U U' | / ( n ulp )               SSKEWSTEVX('V','A', ... )
*>    6= |D(with Z) - EVEIGS| / (|D| ulp)       SSKEWSTEVX('N','A', ... )
*>    7= | A - U S U' | / ( |A| n ulp )         SSTEVR('V','A', ... )
*>    8= | I - U U' | / ( n ulp )               SSTEVR('V','A', ... )
*>    9= |D(with Z) - EVEIGS| / (|D| ulp)       SSTEVR('N','A', ... )
*>    10= | A - U S U' | / ( |A| n ulp )        SSKEWSTEVX('V','I', ... )
*>    11= | I - U U' | / ( n ulp )              SSKEWSTEVX('V','I', ... )
*>    12= |D(with Z) - D(w/o Z)| / (|D| ulp)    SSKEWSTEVX('N','I', ... )
*>    13= | A - U S U' | / ( |A| n ulp )        SSKEWSTEVX('V','V', ... )
*>    14= | I - U U' | / ( n ulp )              SSKEWSTEVX('V','V', ... )
*>    15= |D(with Z) - D(w/o Z)| / (|D| ulp)    SSKEWSTEVX('N','V', ... )
*>    16= | A - U S U' | / ( |A| n ulp )        SSKEWSTEVD('V', ... )
*>    17= | I - U U' | / ( n ulp )              SSKEWSTEVD('V', ... )
*>    18= |D(with Z) - EVEIGS| / (|D| ulp)      SSKEWSTEVD('N', ... )
...
*>
*>    25= | A - U S U' | / ( |A| n ulp )        SSKEWSYEV('L','V', ... )
*>    26= | I - U U' | / ( n ulp )              SSKEWSYEV('L','V', ... )
*>    27= |D(with Z) - D(w/o Z)| / (|D| ulp)    SSKEWSYEV('L','N', ... )
*>    28= | A - U S U' | / ( |A| n ulp )        SSKEWSYEVX('L','V','A', ... )
*>    29= | I - U U' | / ( n ulp )              SSKEWSYEVX('L','V','A', ... )
*>    30= |D(with Z) - D(w/o Z)| / (|D| ulp)    SSKEWSYEVX('L','N','A', ... )
*>    31= | A - U S U' | / ( |A| n ulp )        SSKEWSYEVX('L','V','I', ... )
*>    32= | I - U U' | / ( n ulp )              SSKEWSYEVX('L','V','I', ... )
*>    33= |D(with Z) - D(w/o Z)| / (|D| ulp)    SSKEWSYEVX('L','N','I', ... )
*>    34= | A - U S U' | / ( |A| n ulp )        SSKEWSYEVX('L','V','V', ... )
*>    35= | I - U U' | / ( n ulp )              SSKEWSYEVX('L','V','V', ... )
*>    36= |D(with Z) - D(w/o Z)| / (|D| ulp)    SSKEWSYEVX('L','N','V', ... )
...
*>    61= | A - U S U' | / ( |A| n ulp )        SSKEWSYEVD('L','V', ... )
*>    62= | I - U U' | / ( n ulp )              SSKEWSYEVD('L','V', ... )
*>    63= |D(with Z) - D(w/o Z)| / (|D| ulp)    SSKEWSYEVD('L','N', ... )
...
*
*  =====================================================================
      SUBROUTINE SDRVSKEWST( NSIZES, NN, NTYPES, DOTYPE, ISEED,
...
*     .. External Subroutines ..
     ...
     $                   SLATMS, SSKEWSTEV, SSKEWSTEVX, SSKEWSTT21,
     $                   SSKEWSTT22, SSKEWSYEV, SSKEWSYEVX, SSKEWSTEVD,
     $                   SSKEWSYEVD, SSKEWSYT21, SSKEWSYT22, XERBLA
*     ..
*     .. Scalars in Common ..
      CHARACTER*32       SRNAMT
*     ..
...
*
            ...
            END IF
*
*           3)      If matrix is tridiagonal, call SSKEWSTEV and SSTEVX.
*
            IF( JTYPE.LE.7 ) THEN
               NTEST = 1
               DO 120 I = 1, N
  ...
  130          CONTINUE
               SRNAMT = 'SSKEWSTEV'
               CALL SSKEWSTEV( 'V', N, D1, D2, Z, LDU, WORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEV(V)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
  ...
  140          CONTINUE
               DO 150 I = 1, N - 1
                  D4( I ) = REAL( A( I+1, I ) )
  150          CONTINUE
               CALL SSKEWSTT21( N, 1, D3, D4, D2, D1, Z, LDU, WORK,
     $                      RESULT( 1 ) )
*
               NTEST = 3
               DO 160 I = 1, N - 1
  ...
  160          CONTINUE
               SRNAMT = 'SSKEWSTEV'
               CALL SSKEWSTEV( 'N', N, D3, D4, Z, LDU, WORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEV(N)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
  ...
  200          CONTINUE
               SRNAMT = 'SSKEWSTEVX'
               CALL SSKEWSTEVX( 'V', 'A', N, D2, VL, VU, IL, IU, ABSTOL,
     $                      M, WA1, Z, LDU, WORK, IWORK, IWORK( 5*N+1 ),
     $                      IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEVX(V,A)', IINFO,
     $               N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
  ...
  210          CONTINUE
               DO 220 I = 1, N - 1
                  D4( I ) = REAL( A( I+1, I ) )
  220          CONTINUE
               CALL SSKEWSTT21( N, 1, D3, D4, D2, WA1, Z, LDU, WORK,
     $                      RESULT( 4 ) )
*
               NTEST = 6
               DO 230 I = 1, N - 1
  ...
  230          CONTINUE
               SRNAMT = 'SSKEWSTEVX'
               CALL SSKEWSTEVX( 'N', 'A', N, D4, VL, VU, IL, IU, ABSTOL,
     $                      M2, WA2, Z, LDU, WORK, IWORK,
     $                      IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEVX(N,A)', IINFO,
     $               N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
  ...
  340          CONTINUE
               SRNAMT = 'SSKEWSTEVX'
               CALL SSKEWSTEVX( 'V', 'I', N, D2, VL, VU, IL, IU,
     $                      ABSTOL, M2, WA2, Z, LDU, WORK, IWORK,
     $                      IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEVX(V,I)', IINFO,
     $               N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
  ...
  350          CONTINUE
               DO 360 I = 1, N - 1
                  D4( I ) = REAL( A( I+1, I ) )
  360          CONTINUE
               CALL SSKEWSTT22( N, M2, 1, D3, D4, D2, WA2, Z, LDU,
...
*
  ...
  370          CONTINUE
               SRNAMT = 'SSKEWSTEVX'
               CALL SSKEWSTEVX( 'N', 'I', N, D4, VL, VU, IL, IU,
     $                      ABSTOL, M3, WA3, Z, LDU, WORK, IWORK,
     $                      IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEVX(N,I)', IINFO,
     $               N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
  ...
  400          CONTINUE
               SRNAMT = 'SSKEWSTEVX'
               CALL SSKEWSTEVX( 'V', 'V', N, D2, VL, VU, IL, IU,
     $                      ABSTOL, M2, WA2, Z, LDU, WORK, IWORK,
     $                      IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEVX(V,V)', IINFO,
     $               N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
  ...
  430          CONTINUE
               SRNAMT = 'SSKEWSTEVX'
               CALL SSKEWSTEVX( 'N', 'V', N, D4, VL, VU, IL, IU,
     $                      ABSTOL, M3, WA3, Z, LDU, WORK, IWORK,
     $                      IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEVX(N,V)', IINFO,
     $               N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
  ...
  460          CONTINUE
               SRNAMT = 'SSKEWSTEVD'
               CALL SSKEWSTEVD( 'V', N, D1, D2, Z, LDU, WORK, LWEDC,
     $                      IWORK, LIWEDC, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEVD(V)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
  ...
  490          CONTINUE
               SRNAMT = 'SSKEWSTEVD'
               CALL SSKEWSTEVD( 'N', N, D3, D4, Z, LDU, WORK, LWEDC,
     $                      IWORK, LIWEDC, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSTEVD(N)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
            DO 1720 IUPLO = 0, 1
               ...
               END IF
*
*              4)      Call SSKEWSYEV and SSYEVX.
*
               CALL SLACPY( ' ', N, N, A, LDA, V, LDU )
*
               NTEST = NTEST + 1
               SRNAMT = 'SSKEWSYEV'
               CALL SSKEWSYEV( 'V', UPLO, N, A, LDU, D1, WORK, LWORK,
     $                     IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSYEV(V,' // UPLO //
...
*
               CALL SSKEWSYT21( 1, UPLO, N, 1, V, LDU, D2, D1, A, LDU,
...
*
               NTEST = NTEST + 2
               SRNAMT = 'SSKEWSYEV'
               CALL SSKEWSYEV( 'N', UPLO, N, A, LDU, D3, WORK, LWORK,
     $                     IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSYEV(N,' // UPLO //
     $               ')', IINFO, N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
               SRNAMT = 'SSKEWSYEVX'
               CALL SSKEWSYEVX( 'V', 'A', UPLO, N, A, LDU, VL, VU, IL,
     $                      IU, ABSTOL, M, WA1, Z, LDU, WORK, LWORK,
     $                      IWORK, IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSYEVX(V,A,' // UPLO
     $               // ')', IINFO, N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
               CALL SLACPY( ' ', N, N, V, LDU, A, LDA )
*
               CALL SSKEWSYT21( 1, UPLO, N, 1, A, LDU, D2, WA1, Z, LDU,
     $                      V, LDU, TAU, WORK, RESULT( NTEST ) )
*
               NTEST = NTEST + 2
               SRNAMT = 'SSKEWSYEVX'
               CALL SSKEWSYEVX( 'N', 'A', UPLO, N, A, LDU, VL, VU, IL,
     $                      IU, ABSTOL, M2, WA2, Z, LDU, WORK, LWORK,
     $                      IWORK, IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSYEVX(N,A,' // UPLO
     $               // ')', IINFO, N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
               ...
               CALL SLACPY( ' ', N, N, V, LDU, A, LDA )
               SRNAMT = 'SSKEWSYEVX'
               CALL SSKEWSYEVX( 'V', 'I', UPLO, N, A, LDU, VL, VU, IL,
     $                      IU, ABSTOL, M2, WA2, Z, LDU, WORK, LWORK,
     $                      IWORK, IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSYEVX(V,I,' // UPLO
     $               // ')', IINFO, N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
               CALL SLACPY( ' ', N, N, V, LDU, A, LDA )
*
               CALL SSKEWSYT22( 1, UPLO, N, M2, 1, A, LDU, D2, WA2, Z,
     $                      LDU, V, LDU, TAU, WORK, RESULT( NTEST ) )
*
               NTEST = NTEST + 2
               CALL SLACPY( ' ', N, N, V, LDU, A, LDA )
               SRNAMT = 'SSKEWSYEVX'
               CALL SSKEWSYEVX( 'N', 'I', UPLO, N, A, LDU, VL, VU, IL,
     $                      IU, ABSTOL, M3, WA3, Z, LDU, WORK, LWORK,
     $                      IWORK, IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSYEVX(N,I,' // UPLO
     $               // ')', IINFO, N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
               ...
               CALL SLACPY( ' ', N, N, V, LDU, A, LDA )
               SRNAMT = 'SSKEWSYEVX'
               CALL SSKEWSYEVX( 'V', 'V', UPLO, N, A, LDU, VL, VU, IL,
     $                      IU, ABSTOL, M2, WA2, Z, LDU, WORK, LWORK,
     $                      IWORK, IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSYEVX(V,V,' // UPLO
     $               // ')', IINFO, N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
               ...
               CALL SLACPY( ' ', N, N, V, LDU, A, LDA )
               SRNAMT = 'SSKEWSYEVX'
               CALL SSKEWSYEVX( 'N', 'V', UPLO, N, A, LDU, VL, VU, IL,
     $                      IU, ABSTOL, M3, WA3, Z, LDU, WORK, LWORK,
     $                      IWORK, IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSYEVX(N,V,' // UPLO
     $               // ')', IINFO, N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
               NTEST = NTEST + 1
               SRNAMT = 'SSKEWSYEVD'
               CALL SSKEWSYEVD( 'V', UPLO, N, A, LDU, D1, WORK, LWEDC,
     $                      IWORK, LIWEDC, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSYEVD(V,' // UPLO //
     $               ')', IINFO, N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
               NTEST = NTEST + 2
               SRNAMT = 'SSKEWSYEVD'
               CALL SSKEWSYEVD( 'N', UPLO, N, A, LDU, D3, WORK, LWEDC,
     $                      IWORK, LIWEDC, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'SSKEWSYEVD(N,' // UPLO //
     $               ')', IINFO, N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
            NTESTT = NTESTT + NTEST
*
            CALL SLAFTS( 'SSKEWST', N, N, JTYPE, NTEST, RESULT, IOLDSD,
     $                   THRESH, NOUNIT, NERRS )
*
 1730    CONTINUE
 1740 CONTINUE
...
*     Summary
*
      CALL ALASVM( 'SSKEWST', NOUNIT, NERRS, NTESTT, 0 )
*
 9999 FORMAT( ' SDRVSKEWST: ', A, ' returned INFO=', I6, '.', / 9X,
