...
*  ===========
*
*       SUBROUTINE SDRVSKEWSG2STG( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRESH,
...
*> \verbatim
*>
*>      SDRVSKEWSG2STG checks the real skew-symmetric generalized eigenproblem
*>      drivers.
*>
*>              SSKEWSYGV computes all eigenvalues and, optionally,
*>              eigenvectors of a real skew-symmetric-definite generalized
*>              eigenproblem.
*>
*>              SSKEWSYGVX computes selected eigenvalues and, optionally,
*>              eigenvectors of a real skew-symmetric-definite generalized
*>              eigenproblem.
*>
*>              SSKEWSYGVD computes all eigenvalues and, optionally,
*>              eigenvectors of a real symmetric-definite generalized
*>              eigenproblem using a divide and conquer algorithm.
*>
*>      When SDRVSKEWSG2STG is called, a number of matrix "sizes" ("n's") and a
...
*>
*>      (1) SSKEWSYGV with ITYPE = 1 and UPLO ='U':
*>
*>              | A Z - B Z D | / ( |A| |Z| n ulp )
*>              | D - D2 | / ( |D| ulp )   where D is computed by
*>                                         SSKEWSYGV and  D2 is computed by
*>                                         SSKEWSYGV_2STAGE. This test is
*>                                         only performed for SSKEWSYGV
*>
...
*>
*>      (7) SSKEWSYGV with ITYPE = 2 and UPLO ='U':
*>
*>              | A B Z - Z D | / ( |A| |Z| n ulp )
*>
...
*>
*>      (11) SSKEWSYGV with ITYPE = 3 and UPLO ='U':
*>
*>              | B A Z - Z D | / ( |A| |Z| n ulp )
*>
...
*>
*>      SSKEWSYGVD, SSPGVD and SSBGVD performed the same 14 tests.
*>
*>      SSKEWSYGVX, SSPGVX and SSBGVX performed the above 14 tests with
...
*>          SDRVSKEWSG2STG does nothing.  It must be at least zero.
*>          Not modified.
*>
...
*>
*>  NTYPES  INTEGER
*>          The number of elements in DOTYPE.   If it is zero, SDRVSKEWSG2STG
...
*>          next call to SDRVSKEWSG2STG to continue the same random number
*>          sequence.
*>          Modified.
*>
*>  THRESH  REAL
...
*>          -21: NWORK too small.
*>          -23: LIWORK too small.
*>          If  SLATMR, SLATMS, SSKEWSYGV, SSPGV, SSBGV, SSKEWSYGVD, SSPGVD,
*>              SSBGVD, SSKEWSYGVX, SSPGVX or SSBGVX returns an error code,
...
*
*  =====================================================================
      SUBROUTINE SDRVSKEWSG2STG( NSIZES, NN, NTYPES, DOTYPE, ISEED,
     $                       THRESH, NOUNIT, A, LDA, B, LDB, D, D2,
     $                       Z, LDZ, AB, BB, AP, BP, WORK, NWORK,
...
*     .. External Subroutines ..
     ...
     $                   SLATMS, SSKEWSYGV, SSKEWSYGVX, SSKEWSYGVD,
     $                   SSKEWSGT01, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL XERBLA( 'SDRVSKEWSG2STG', -INFO )
         RETURN
      END IF
*
...
*
            ...
            END IF
*
*           3) Call SSKEWSYGV, SSPGV, SSBGV, SSKEWSYGVD, SSPGVD, SSBGVD,
*              SSKEWSYGVX, SSPGVX, and SSBGVX, do tests.
*
...
*
                  CALL SSKEWSYGV( IBTYPE, 'V', UPLO, N, Z, LDZ, BB,
     $                        LDB, D, WORK, NWORK, IINFO )
*
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'SSKEWSYGV(V,' // UPLO
...
*                 Do Test
*
                  CALL SSKEWSGT01( IBTYPE, UPLO, N, N, A, LDA, B, LDB,
     $                         Z, LDZ, D, WORK, RESULT( NTEST ) )
*
*                 Test SSKEWSYGVD
*
                  NTEST = NTEST + 1
*
                  ...
                  CALL SLACPY( UPLO, N, N, B, LDB, BB, LDB )
*
                  CALL SSKEWSYGVD( IBTYPE, 'V', UPLO, N, Z, LDZ, BB,
     $                         LDB, D, WORK, NWORK, IWORK, LIWORK,
     $                         IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'SSKEWSYGVD(V,' // UPLO
     $                  // ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
                  ...
                  CALL SLACPY( UPLO, N, N, B, LDB, BB, LDB )
*
                  CALL SSKEWSYGVX( IBTYPE, 'V', 'A', UPLO, N, AB, LDA,
     $                         BB, LDB, VL, VU, IL, IU, ABSTOL, M, D,
     $                         Z, LDZ, WORK, NWORK, IWORK( N+1 ),
     $                         IWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'SSKEWSYGVX(V,A' // UPLO
     $                  // ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
                  VL = ZERO
                  VU = ANORM
                  CALL SSKEWSYGVX( IBTYPE, 'V', 'V', UPLO, N, AB, LDA,
     $                         BB, LDB, VL, VU, IL, IU, ABSTOL, M, D,
     $                         Z, LDZ, WORK, NWORK, IWORK( N+1 ),
     $                         IWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'SSKEWSYGVX(V,V,' //
...
*                 Do Test
*
                  CALL SSKEWSGT01( IBTYPE, UPLO, N, M, A, LDA, B, LDB,
     $                         Z, LDZ, D, WORK, RESULT( NTEST ) )
*
                  NTEST = NTEST + 1
*
                  ...
                  CALL SLACPY( UPLO, N, N, B, LDB, BB, LDB )
*
                  CALL SSKEWSYGVX( IBTYPE, 'V', 'I', UPLO, N, AB, LDA,
     $                         BB, LDB, VL, VU, IL, IU, ABSTOL, M, D,
     $                         Z, LDZ, WORK, NWORK, IWORK( N+1 ),
     $                         IWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'SSKEWSYGVX(V,I,' //
     $                  UPLO // ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
      RETURN
*
*     End of SDRVSKEWSG2STG
*
 9999 FORMAT( ' SDRVSKEWSG2STG: ', A, ' returned INFO=', I6, '.', / 9X,
