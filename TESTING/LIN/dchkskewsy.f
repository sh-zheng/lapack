...
*  ===========
*
*       SUBROUTINE DCHKSKEWSY( DOTYPE, NN, NVAL, NNB, NBVAL, NNS, NSVAL,
...
*> \verbatim
*>
*> DCHKSKEWSY tests DSKEWSYTRF, -TRI2, -TRS, -TRS2, -RFS, and -CON.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*
*  =====================================================================
      SUBROUTINE DCHKSKEWSY( DOTYPE, NN, NVAL, NNB, NBVAL, NNS,
...
*     ..
*     .. External Functions ..
      DOUBLE PRECISION   DGET06, DLANSKEWSY
      EXTERNAL           DGET06, DLANSKEWSY
*     ..
*     .. External Subroutines ..
      EXTERNAL           ALAERH, ALAHD, ALASUM, DERRSY, DGET04, DLACPY,
     $                   DLARHS, DLATB4, DLATMS, DPOT08, DPOT07, DPOT09,
     $                   DERRSKEWSY, DSKEWSYCON, DSKEWSYCONV,
     $                   DSKEWSYRFS, DSKEWSYT01, DSKEWSYTRF,
     $                   DSKEWSYTRI2, DSKEWSYTRS, DSKEWSYTRS2, LSAME,
     $                   XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
                  LWORK = MAX( 2, NB )*LDA
                  SRNAMT = 'DSKEWSYTRF'
                  CALL DSKEWSYTRF( UPLO, N, AFAC, LDA, IWORK, AINV,
     $                         LWORK, INFO )
*
...
*
  ...
  100                CONTINUE
     ...
     $                  THEN
                  ...
                  END IF
*
*                 Check error code from DSKEWSYTRF and handle error.
*
                  IF( INFO.NE.K )
     $               CALL ALAERH( PATH, 'DSKEWSYTRF', INFO, K, UPLO,
     $                            N, N, -1, -1, NB, IMAT, NFAIL,
     $                            NERRS, NOUT )
*
...
*
                  CALL DSKEWSYT01( UPLO, N, A, LDA, AFAC, LDA,
...
*
                  IF( INB.EQ.1.AND..NOT.TRFCON ) THEN
                     ...
                     SRNAMT = 'DSKEWSYTRI2'
                     LWORK = (N+NB+1)*(NB+3)
                     CALL DSKEWSYTRI2( UPLO, N, AINV, LDA, IWORK, WORK,
     $                            LWORK, INFO )
*
*                    Check error code from DSKEWSYTRI2 and handle error.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'DSKEWSYTRI2', INFO, -1,
     $                               UPLO, N, N, -1, -1, -1, IMAT,
     $                               NFAIL, NERRS, NOUT )
*
...
*
                     SRNAMT = 'DSKEWSYTRS'
                     CALL DSKEWSYTRS( UPLO, N, NRHS, AFAC, LDA, IWORK,
     $                            X, LDA, INFO )
*
*                    Check error code from DSKEWSYTRS and handle error.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'DSKEWSYTRS', INFO, 0, UPLO,
...
*
                     SRNAMT = 'DSYTRS2'
                     CALL DSKEWSYTRS2( UPLO, N, NRHS, AFAC, LDA, IWORK,
     $                            X, LDA, WORK, INFO )
*
*                    Check error code from DSKEWSYTRS2 and handle error.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'DSKEWSYTRS2', INFO, 0, UPLO,
     $                               N, N, -1, -1, NRHS, IMAT, NFAIL,
     $                               NERRS, NOUT )
*
                     CALL DLACPY( 'Full', N, NRHS, B, LDA, WORK, LDA )
...
*
                     SRNAMT = 'DSKEWSYRFS'
                     CALL DSKEWSYRFS( UPLO, N, NRHS, A, LDA, AFAC,
     $                            LDA, IWORK, B, LDA, X, LDA, RWORK,
     $                            RWORK( NRHS+1 ), WORK, IWORK( N+1 ),
     $                            INFO )
*
*                    Check error code from DSKEWSYRFS and handle error.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'DSKEWSYRFS', INFO, 0, UPLO,
     $                               N, N, -1, -1, NRHS, IMAT, NFAIL,
     $                               NERRS, NOUT )
*
                     CALL DGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
  140             CONTINUE
                  ANORM = DLANSKEWSY( '1', UPLO, N, A, LDA, RWORK )
                  SRNAMT = 'DSKEWSYCON'
                  CALL DSKEWSYCON( UPLO, N, AFAC, LDA, IWORK, ANORM,
     $                         RCOND, WORK, IWORK( N+1 ), INFO )
*
*                 Check error code from DSKEWSYCON and handle error.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'DSKEWSYCON', INFO, 0, UPLO, N,
