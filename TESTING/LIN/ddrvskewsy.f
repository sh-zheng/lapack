...
*  ===========
*
*       SUBROUTINE DDRVSKEWSY( DOTYPE, NN, NVAL, NRHS, THRESH, TSTERR, NMAX,
...
*> \verbatim
*>
*> DDRVSKEWSY tests the driver routines DSKEWSYSV.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*
*  =====================================================================
      SUBROUTINE DDRVSKEWSY( DOTYPE, NN, NVAL, NRHS, THRESH,
     $                   TSTERR, NMAX, A, AFAC, AINV, B, X, XACT,
...
*     .. External Subroutines ..
     ...
     $                   DSKEWSYSV, DSKEWSYSVX, DSKEWSYT01, DSKEWSYTRF,
     $                   DSKEWSYTRI2, LSAME, XLAENV
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
      CHARACTER*32       SRNAMT
...
*                 the value returned by DSKEWSYSVX.
*
                  IF( ZEROT ) THEN
                     IF( IFACT.EQ.1 ) THEN
                        LWORK = (N+NB+1)*(NB+3)
...
*
                     ...
                     CALL DSKEWSYTRF( UPLO, N, AFAC, LDA, IWORK, WORK,
     $                            LWORK, INFO )
                     BIF = INFO
*
...
*
     ...
     $                            LWORK, INFO )
                     AINVNM = DLANSKEWSY( '1', UPLO, N, AINV, LDA,
     $                                    RWORK )
*
...
*
*                    Factor the matrix and solve the system using DSKEWSYSV.
*
                     SRNAMT = 'DSKEWSYSV '
                     CALL DSKEWSYSV( UPLO, N, NRHS, AFAC, LDA, IWORK, X,
     $                           LDA, WORK, LWORK, INFO )
*
...
*
  ...
  100                   CONTINUE
     ...
     $                     THEN
                     ...
                     END IF
*
*                    Check error code from DSKEWSYSV .
*
                     IF( INFO.NE.K ) THEN
                        CALL ALAERH( PATH, 'DSKEWSYSV ', INFO, K, UPLO,
...
*                    residual.
*
                     CALL DSKEWSYT01( UPLO, N, A, LDA, AFAC, LDA, IWORK,
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'DSKEWSYSV ', UPLO,
     $                        N, IMAT, K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
  110                CONTINUE
...
*                 error bounds using DSKEWSYSVX.
*
                  SRNAMT = 'DSKEWSYSVX'
                  CALL DSKEWSYSVX( FACT, UPLO, N, NRHS, A, LDA, AFAC,
     $                         LDA, IWORK, B, LDA, X, LDA, RCOND,
     $                         RWORK, RWORK( NRHS+1 ), WORK, LWORK,
     $                         IWORK( N+1 ), INFO )
*
*                 Adjust the expected value of INFO to account for
*                 pivoting.
*
  ...
  130                CONTINUE
     ...
     $                  THEN
                  ...
                  END IF
*
*                 Check the error code from DSKEWSYSVX.
*
                  IF( INFO.NE.K ) THEN
                     CALL ALAERH( PATH, 'DSKEWSYSVX', INFO, K,
...
*
     ...
     $                            RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from DSKEWSYSVX with the computed value
*                 in RCONDC.
*
...
*
                  DO 140 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'DSKEWSYSVX', FACT,
